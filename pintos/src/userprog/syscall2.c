...
static bool sys_create1(const char *file, unsigned initial_size);
static bool sys_remove1(const char *file);
static int sys_open1(const char *file);
static void sys_close1(int fd);
void sys_exit1(int status);
static int sys_write1(int fd, const void *buffer, unsigned size);
static int sys_read1(int fd, void *buffer, unsigned size);
static int sys_filesize1(int fd);
static pid_t sys_exec1(const char *cmd_line);
static int sys_wait1(pid_t pid);
static void sys_seek1(int fd, unsigned position);
static unsigned sys_tell1(int fd);
static void sys_halt1(void);
static int sys_mmap1(int fd, void *addr);
static void sys_munmap1(int map);
...
check_and_pin_buffer(void *uaddr, unsigned int len, void *esp, bool write)
{
  ...
  {
    if (!is_valid_user_addr (addr)) sys_exit1 (-1);
    struct spt_entry *spte = check_and_pin_addr (addr, esp);
    if (spte != NULL && write && !spte->writeable) sys_exit1 (-1);
  }
}
...
syscall_handler(struct intr_frame *f UNUSED)
{
  ...
  case SYS_CREATE:
    ...
    f->eax = sys_create1((char *)syscall_args[0], syscall_args[1]);
  ...
  case SYS_REMOVE:
    ...
    check_and_pin_string ((const void *)syscall_args[0], f->esp);
    f->eax = sys_remove1((char *)syscall_args[0]);
    break;
  case SYS_OPEN:
    get_syscall_arg(f, syscall_args, 1);
    check_and_pin_string ((const void *)syscall_args[0], f->esp);
    f->eax = sys_open1((char *)syscall_args[0]);
    ASSERT (is_valid_user_addr (syscall_args[0]));
    unpin_string ((void *)syscall_args[0]);
    break;
  case SYS_CLOSE:
    get_syscall_arg(f, syscall_args, 1);
    sys_close1(syscall_args[0]);
    break;
  case SYS_EXIT:
    get_syscall_arg(f, syscall_args, 1);
    sys_exit1(syscall_args[0]);
    break;
  case SYS_WRITE:
    get_syscall_arg(f, syscall_args, 3);
    check_and_pin_buffer ((void *)syscall_args[1], syscall_args[2], f->esp, false);
    f->eax = sys_write1(syscall_args[0], (void *)syscall_args[1], syscall_args[2]);
    unpin_buffer ((void *)syscall_args[1], syscall_args[2]);
    break;
  case SYS_READ:
    get_syscall_arg(f, syscall_args, 3);
    check_and_pin_buffer ((void *)syscall_args[1], syscall_args[2], f->esp, true);
    f->eax = sys_read1(syscall_args[0], (void *)syscall_args[1], syscall_args[2]);
    unpin_buffer ((void *)syscall_args[1], syscall_args[2]);
    break;
  case SYS_FILESIZE:
    get_syscall_arg(f, syscall_args, 1);
    f->eax = sys_filesize1(syscall_args[0]);
    break;
  case SYS_EXEC:
    get_syscall_arg(f, syscall_args, 1);
    check_and_pin_string ((const void *)syscall_args[0], f->esp);
    f->eax = sys_exec1((char *)syscall_args[0]);
    ASSERT (is_valid_user_addr (syscall_args[0]));
    unpin_string ((void *)syscall_args[0]);
    break;
  case SYS_WAIT:
    get_syscall_arg(f, syscall_args, 1);
    f->eax = sys_wait1(syscall_args[0]);
    break;
  case SYS_SEEK:
    get_syscall_arg(f, syscall_args, 2);
    sys_seek1(syscall_args[0], syscall_args[1]);
    break;
  case SYS_TELL:
    get_syscall_arg(f, syscall_args, 1);
    f->eax = sys_tell1(syscall_args[0]);
    break;
  case SYS_MMAP:
    get_syscall_arg(f, syscall_args, 2);
    f->eax = sys_mmap1(syscall_args[0], (void *)syscall_args[1]);
    break;
  case SYS_MUNMAP:
    get_syscall_arg(f, syscall_args, 1);
    sys_munmap1(syscall_args[0]);
    break;
  case SYS_HALT:
    sys_halt1();
    break;
...
static bool
sys_create1(const char *file, unsigned initial_size)
{
...
static int
sys_write1(int fd, const void *buffer, unsigned size)
{
  lock_acquire(&filesys_lock);
  if (fd == 0)
  {
...
static int
sys_read1(int fd, void *buffer, unsigned size)
{ 
  lock_acquire(&filesys_lock);
  if (fd == 0)
  {
...
static void
sys_seek1(int fd, unsigned position)
{
  struct file_node *fd_s = get_fdstruct(fd);
  if (!fd_s)
    return;
